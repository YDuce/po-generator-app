...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column('drive_folder_id', sa.String(length=256), nullable=False, unique=True),
    )
    op.create_table(
        'user',
        sa.Column('id', sa.Integer(), primary_key=True),
    ...
    op.create_table(
        ...
        sa.Column('channel', sa.String(length=50), nullable=False),
        sa.Column('product_sku', sa.String(length=80), sa.ForeignKey('product.sku'), nullable=False),
        sa.Column('quantity', sa.Integer(), nullable=True),
