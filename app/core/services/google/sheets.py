...
from app.core.services.sheets import SheetMetadata
...
class GoogleSheetsService:
    ...
    def get_sheet_data(self, spreadsheet_id: str, range_name: str) -> List[List[str]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get data from sheet {spreadsheet_id} range {range_name}: {error}"
            )
            raise
    ...
    def update_sheet_data(
        ...
        try:
            ...
            logger.error(
                f"Failed to update sheet {spreadsheet_id} range {range_name}: {error}"
            )
            raise
    ...
    def append_sheet_data(
        ...
        try:
            ...
            logger.error(
                f"Failed to append data to sheet {spreadsheet_id} range {range_name}: {error}"
            )
            raise
    ...
    def clear_sheet_data(self, spreadsheet_id: str, range_name: str) -> SheetResponse:
        ...
        try:
            ...
            logger.error(
                f"Failed to clear data from sheet {spreadsheet_id} range {range_name}: {error}"
            )
            raise
