    ...
    access_token = jwt.encode(
        {
            ...
            + datetime.timedelta(seconds=ACCESS_EXPIRES),
        },
        JWT_SECRET,
        algorithm=JWT_ALGO,
    )
    refresh_token = jwt.encode(
        {
            "sub": user.id,
            "exp": datetime.datetime.utcnow()
            + datetime.timedelta(seconds=REFRESH_EXPIRES),
        },
        JWT_SECRET,
        algorithm=JWT_ALGO,
    )
...
def google_login():
    ...
    return (
        jsonify(
            {
                "message": f"Google OAuth login not implemented. Client ID: {GOOGLE_CLIENT_ID}"
            }
        ),
        501,
    )
...
def google_callback():
    ...
    return (
        jsonify({"message": "Google OAuth callback not implemented in this stub."}),
        501,
    )
